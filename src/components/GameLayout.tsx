...
export function GameLayout({ children, userProfile, showBackButton = false }: GameLayoutProps) {
  ...
  // Hide floating UI on dashboard and admin pages
  ...
  return (
    <div className="relative min-h-screen">
      ...
      {/* Modern Floating Navigation - Hidden on Dashboard and Admin */}
            ...
            <div className="glass-card glass-hover rounded-2xl p-5 max-w-xs motion-safe:animate-float shadow-md">
              ...
              {isAdmin && (
                <button
                  ...
                  className="mt-3 w-full bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-700 hover:to-teal-700 text-white px-5 py-3 rounded-xl font-semibold transition-all duration-300 hover:scale-105 shadow-md group"
                >
                  <div className="flex items-center justify-center space-x-3">
                    <svg className="h-5 w-5 group-hover:scale-110 transition-transform duration-200" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 11c1.657 0 3-1.567 3-3.5S13.657 4 12 4 9 5.567 9 7.5 10.343 11 12 11zm0 2c-2.21 0-6 1.12-6 3.333V19a1 1 0 001 1h10a1 1 0 001-1v-2.667C18 14.12 14.21 13 12 13z" />
                    </svg>
                    <span>Admin</span>
                    <svg className="h-4 w-4 group-hover:translate-x-1 transition-transform duration-200" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
      ...
      {/* Mobile-only floating avatar button (bottom-right) - Hidden on Dashboard and Admin */}
      {!isDashboardPage && !isAdminPage && (
        <div className="fixed bottom-4 right-4 z-40 md:hidden">
          ...
          {isAdmin && (
            <button
              ...
              className="mt-3 w-12 h-12 rounded-full shadow-md bg-gradient-to-br from-emerald-600 to-teal-600/90 flex items-center justify-center text-white font-bold"
            >
              A
            </button>
          )}
